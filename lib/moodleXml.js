...
export function buildMoodleXml(questions) {
  ...
  // Format code-like reference solutions for nicer display inside CDATA.
  // - normalize line endings
  // - insert line breaks after semicolons and around braces when compact
  // - collapse excessive blank lines
  const formatCode = (raw='') => {
    if (!raw) return '';
    let s = String(raw).replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    // If already has multiple lines, preserve them but still normalize braces
    // Insert newline after semicolons if not already a newline
    s = s.replace(/;\s*/g, ';\n');
    // Put opening brace on same line but ensure a newline after
    s = s.replace(/\{\s*/g, '{\n');
    // Ensure closing braces start on a new line
    s = s.replace(/\s*\}/g, '\n}');
  ...
  const items = questions.map((q, idx) => {
    ...
    if (q.type === 'coderunner') {
      ...
      <question type="essay">
        ...
        <questiontext format="html"><text><![CDATA[${q.text}]]></text></questiontext>
        <generalfeedback>
          <text><![CDATA[<pre><code>${refEsc}</code></pre>]]></text>
        </generalfeedback>
        <graderinfo>
          <text><![CDATA[<pre><code>${refEsc}</code></pre>]]></text>
        </graderinfo>
